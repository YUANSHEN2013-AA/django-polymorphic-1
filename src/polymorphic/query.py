...
class PolymorphicModelIterable(BasePolymorphicModelIterable, Generic[_All, _Base]):
    ...
    async def __aiter__(self) -> AsyncIterator[_All]:
        base_iter = super().__aiter__()
        if self.queryset.polymorphic_disabled:
            async for obj in base_iter:
                yield obj
            return
        
        async for obj in self._apolymorphic_iterator(base_iter):
            yield obj
    ...
    async def _apolymorphic_iterator(self, base_iter: AsyncIterator[_All]) -> AsyncIterator[_All]:
        ...
        while True:
            ...
            if base_result_objects:
                real_results = await self.queryset._aget_real_instances(base_result_objects)
...
class PolymorphicQuerySet(QuerySet[_All], Generic[_All, _Base]):
    ...
    async def aget_real_instances(self, base_result_objects: Iterable[_All] | None = None) -> list[_All]:
        if base_result_objects is None:
            base_result_objects = [obj async for obj in self]
        base_result_list: Sequence[_All]
        if not isinstance(base_result_objects, list):
            base_result_list = list(base_result_objects)
        else:
    ...
    async def aget(self, *args: Any, **kwargs: Any) -> _All:
        ...
        if clone.query.is_empty():
            raise self.model.DoesNotExist(
                "%s matching query does not exist." % self.model._meta.object_name
            )
        clone.query.set_limits(high=2)
        objs = []
    ...
    async def alast(self) -> _All | None:
        ...
        if not clone.query.order_by:
            clone.query.add_ordering(clone.model._meta.pk.name)
    ...
    def aiterator(self, chunk_size: int = 2000) -> AsyncIterator[_All]:
        clone = self._clone()
        clone.chunk_size = chunk_size
        clone.chunked_fetch = True
        return clone.__aiter__()
    ...
    async def _aget_real_instances(self, base_result_objects: Sequence[_All]) -> list[_All]:
        ...
        get_for_model_async = sync_to_async(content_type_manager.get_for_model)
        get_for_id_async = sync_to_async(content_type_manager.get_for_id)
        
        self_model_class_ct = await get_for_model_async(self.model, for_concrete_model=False)
        self_model_class_id = self_model_class_ct.pk
        
        self_concrete_model_class_ct = await get_for_model_async(self.model, for_concrete_model=True)
        self_concrete_model_class_id = self_concrete_model_class_ct.pk
        ...
        for i, base_object in enumerate(base_result_objects):
            if base_object.polymorphic_ctype_id == self_model_class_id:
                ...
                if real_concrete_class_id is None:
                    ...
                    real_concrete_class_ct = await get_for_id_async(real_concrete_class_id)
                    real_concrete_class = cast("type[_All] | None", real_concrete_class_ct.model_class())
                    if real_concrete_class is not None:
                        if real_concrete_class not in idlist_per_model:
                            heapq.heappush(
                                classes_to_query,
        ...
        while classes_to_query:
            ...
            idlist = idlist_per_model.pop(real_concrete_class)
            indices = indexlist_per_model.pop(real_concrete_class)
            real_objects_qs = real_concrete_class._base_objects.db_manager(self.db).filter(
                **{(f"{pk_name}__in"): idlist}
            )
            real_objects_qs.query.select_related = self.query.select_related
            ...
            for field in existing_fields:
                ...
                deferred_loading_fields.append(translated_field_name)
            real_objects_qs.query.deferred_loading = (
                set(deferred_loading_fields),
                self.query.deferred_loading[1],
            )

            real_objects = []
            async for obj in real_objects_qs:
